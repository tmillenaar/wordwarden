...
## General usage
```target/debug/word_warden <file1> <file2> <dir1> <dir2> 'word1' 'word2' 'word3'```
...
If you have a word you want to check for that happens to match a local file or directory name, use `-w` as a prefix to force interpretation as a target word, like so "word_warden <file1> -w 'my_target_word'".

By default, some colored highlighting is applied to the output. If your environment does not support this, you can turn it off with the flag `--no-highlighting`.
